...
namespace GUI
{
    public class DashboardControlStock : Form, IIdiomaObserver
    {
        ...
        {
            _panelHeader = new Panel { Dock = DockStyle.Top, Height = 62, BackColor = Color.FromArgb(34, 100, 60) };
            _panelHeader.Paint += (s, pe) =>
            {
                using (var br = new LinearGradientBrush(_panelHeader.ClientRectangle,
                    Color.FromArgb(15, 60, 30), Color.FromArgb(34, 100, 60), LinearGradientMode.Horizontal))
            ...
            _btnRefrescar = new Button { Text = "↻  Actualizar", Size = new Size(100, 28), Anchor = AnchorStyles.Top | AnchorStyles.Right, FlatStyle = FlatStyle.Flat, BackColor = Color.FromArgb(50, 140, 80), ForeColor = Color.White, Font = new Font("Segoe UI", 8.5f), Cursor = Cursors.Hand, Location = new Point(756, 17) };
            _btnRefrescar.FlatAppearance.BorderColor = Color.FromArgb(180, 100, 200, 140);
            ...
            var lblSub = new Label { Text = "WardrobeFlow  —  Stock", Font = new Font("Segoe UI", 8f, FontStyle.Italic), ForeColor = Color.FromArgb(200, 220, 255, 220), AutoSize = true, Location = new Point(14, 36), BackColor = Color.Transparent };
            _panelHeader.Controls.AddRange(new Control[] { _lblTitulo, lblSub, _btnRefrescar });

            var panelSbar = new Panel { Dock = DockStyle.Bottom, Height = 26, BackColor = Color.FromArgb(15, 60, 30) };
            _lblSesion = new Label { Dock = DockStyle.Fill, ForeColor = Color.FromArgb(180, 210, 195), Font = new Font("Segoe UI", 8f), TextAlign = ContentAlignment.MiddleLeft, Padding = new Padding(10, 0, 0, 0) };
