...
| Problema | Sev. | Evidencia | Impacto | Recomendación |
|---|---|---|---|---|
| **Tabla `RolPermiso` legacy** (deprecación documentada) | Media→Baja | `DAL/Permiso.cs` (nota de clase) + `CREATE TABLE` en ambos scripts | **Análisis confirmó**: nada la *escribe* en runtime y solo se *lee* en ramas de fallback (BD sin migrar); la fuente de verdad de autorización es `PermisoRelacion`. Su único rol vivo es **seed/bootstrap**: de sus asignaciones planas se generan los nodos-rol y aristas del Composite. Se documentó explícitamente como tal (Opción A). Eliminación física pendiente para una iteración futura (requiere reescribir el seed sin el intermediario). |
