-- ============================================================
-- WardrobeFlowDB — RESET COMPLETO (v2.2)
-- ============================================================
-- Ejecutar este único script en SSMS para volver al estado
-- inicial con datos de prueba. Incorpora todos los cambios
-- de los parches v2.1 y v2.2.
--
-- ¿Qué hace?
--   1. Elimina la BD existente (si existe)
--   2. La recrea con el esquema completo actualizado
--   3. Carga todos los datos de prueba
--
-- USUARIOS DE PRUEBA:
...
INSERT INTO RolPermiso (Rol, IdPermiso) VALUES
('Vendedor', 3),   -- Ver Prendas       (M03 — catálogo)
('Vendedor', 7),   -- Gestionar Clientes (M02)
('Vendedor', 8),   -- Gestionar Planes   (M02)
('Vendedor', 9);   -- Realizar Ventas    (M04)
GO
...
-- ============================================================
-- USUARIOS DE PRUEBA (v2.2: operador → OperadorDeInventario)
-- ============================================================
...
(3, 'operador',
 ...
 'OperadorDeInventario', 1, 'Operador de Inventario'),
(4, 'vendedor',
...
(DATEADD(DAY,-60,GETDATE()), 4,    'Login',            'Inicio Sesion',        'Login exitoso. Usuario: vendedor a las 09:15:00.', 0),
(DATEADD(DAY,-55,GETDATE()), 4,    'Login',            'Inicio Sesion',        'Login exitoso. Usuario: vendedor a las 09:02:33.', 0),
(DATEADD(DAY,-50,GETDATE()), 5,    'Login',            'Inicio Sesion',        'Login exitoso. Usuario: stock a las 08:50:11.', 0),
(DATEADD(DAY,-45,GETDATE()), 3,    'Login',            'Inicio Sesion',        'Login exitoso. Usuario: operador a las 10:30:00.', 0),
(DATEADD(DAY,-40,GETDATE()), 4,    'Login',            'Inicio Sesion',        'Login exitoso. Usuario: vendedor a las 09:10:45.', 0),
(DATEADD(DAY,-30,GETDATE()), 2,    'Login',            'Inicio Sesion',        'Login exitoso. Usuario: supervisor a las 14:00:00.', 0),
(DATEADD(DAY,-15,GETDATE()), 1,    'Login',            'Inicio Sesion',        'Login exitoso. Usuario: admin a las 08:00:00.', 0),
(DATEADD(DAY,-60,GETDATE()), 1,    'Gestion Usuarios', 'Alta Usuario',         'Alta Usuario: ''vendedor'' [Vendedor]', 2),
(DATEADD(DAY,-60,GETDATE()), 1,    'Gestion Usuarios', 'Alta Usuario',         'Alta Usuario: ''stock'' [ControladorDeStock]', 2),
(DATEADD(DAY,-59,GETDATE()), 1,    'Gestion Usuarios', 'Alta Usuario',         'Alta Usuario: ''operador'' [OperadorDeInventario]', 2),
(DATEADD(DAY,-59,GETDATE()), 1,    'Gestion Usuarios', 'Alta Usuario',         'Alta Usuario: ''supervisor'' [Supervisor]', 2),
(DATEADD(DAY,-60,GETDATE()), 4,    'Clientes',         'Alta Cliente',         'Alta Cliente: Sofia Ramirez (DNI 11111111)', 1),
(DATEADD(DAY,-55,GETDATE()), 4,    'Clientes',         'Alta Cliente',         'Alta Cliente: Valentina Torres (DNI 22222222)', 1),
(DATEADD(DAY,-50,GETDATE()), 4,    'Clientes',         'Alta Cliente',         'Alta Cliente: Agustina Perez (DNI 33333333)', 1),
(DATEADD(DAY,-40,GETDATE()), 4,    'Clientes',         'Alta Cliente',         'Alta Cliente: Camila Rodriguez (DNI 44444444)', 1),
(DATEADD(DAY,-20,GETDATE()), NULL, 'Login',            'Intento Fallido Login','Intento fallido #1/3 para ''vendedor'' a las 11:23:10.', 4),
(DATEADD(DAY,-20,GETDATE()), NULL, 'Login',            'Intento Fallido Login','Intento fallido #2/3 para ''vendedor'' a las 11:23:45.', 4),
(DATEADD(DAY,-20,GETDATE()), NULL, 'Login',            'Intento Fallido Login','Intento fallido #3/3 para ''vendedor'' a las 11:24:02.', 4),
(DATEADD(DAY,-20,GETDATE()), NULL, 'Login',            'Bloqueo de Cuenta',    'Cuenta ''vendedor'' bloqueada tras 3 intentos fallidos a las 11:24:02.', 6),
(DATEADD(DAY,-20,GETDATE()), 1,    'Gestion Usuarios', 'Desbloqueo de Cuenta', 'Desbloqueo de Cuenta: ''vendedor'' — Administrador.', 3),
(DATEADD(DAY,-10,GETDATE()), 1,    'Gestion Usuarios', 'Reset Contrasena',     'Contrasena reseteada para usuario ID 4 (vendedor).', 5),
(DATEADD(DAY,-35,GETDATE()), 4,    'Clientes',         'Modificar Cliente',    'Modificar Cliente ID 3: Agustina Perez', 2),
(DATEADD(DAY,-25,GETDATE()), 5,    'Prendas',          'Modificar Prenda',     'Modificar Prenda ID 1: Vestido floral verano', 2),
(DATEADD(DAY,-15,GETDATE()), 3,    'Prendas',          'Estado Prenda',        'Prenda ID 20: EnUso → EnLimpieza', 2),
(DATEADD(DAY,-15,GETDATE()), 3,    'Prendas',          'Estado Prenda',        'Prenda ID 21: EnUso → EnLimpieza', 2),
(DATEADD(DAY,-30,GETDATE()), 5,    'Prendas',          'Estado Prenda',        'Prenda ID 25 ''Crop top lycra rosa'': Disponible → Baja', 3),
(DATEADD(DAY,-60,GETDATE()), 4,    'Login',            'Cierre Sesion',        'Cierre de sesion. Usuario: vendedor a las 18:30:00.', 0),
(DATEADD(DAY,-15,GETDATE()), 1,    'Login',            'Cierre Sesion',        'Cierre de sesion. Usuario: admin a las 17:00:00.', 0),
(GETDATE(),                  1,    'Login',            'Inicio Sesion',        'Login exitoso. Usuario: admin.', 0);
GO
...
PRINT '=== Reset completo. WardrobeFlowDB lista v2.2 ===';
